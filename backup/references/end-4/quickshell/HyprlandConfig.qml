// Servicio Quickshell para modificar config de Hyprland en runtime — end-4/dots-hyprland
// Escribe en un archivo Lua separado (shellOverrides/main.lua) via hyprconfigurator.py.
// Escucha el evento "configreloaded" de Hyprland para propagar cambios.
//
// Uso desde otros QML:
//   HyprlandConfig.set("decoration:rounding", 12)
//   HyprlandConfig.setMany({"decoration:screen_shader": path, "debug:damage_tracking": 1})
//   HyprlandConfig.reset("decoration:screen_shader")
pragma Singleton
...
Singleton {
    ...
    Connections {
        ...
        function onRawEvent(event) {
            if (event.name == "configreloaded") root.reloaded()
        }
    }
}
