...
//
// Ported from the SDDM "silent" theme's custom KeyboardStyle
// (components/QtQuick/VirtualKeyboard/Styles/vkeyboardStyle/style.qml) for
// this project's lockscreen. Every `Config.` reference became `LockConfig.`
// (this project's singleton exposes the same property names as the theme's
// own Config.qml) and every `resourcePrefix + "name.svg"` became
// `LockConfig.getIcon("name.svg")`. Sizing is left untouched: this file has
// no `Config.generalScale` references at all — every size here derives from
// `scaleHint`, a property built into Qt's KeyboardStyle base type that Qt
// computes automatically from the keyboard's actual on-screen size relative
// to `keyboardDesignWidth`/`keyboardDesignHeight` below. The DPI/resolution
// independence this project's other modules/lock/*.qml files get from
// manually multiplying by `(Screen.width / 1920)` is already achieved here
// transitively, since LockVirtualKeyboard.qml's InputPanel.width (the
// container Qt sizes this style's design canvas against) already applies
// that same factor — adding it again in here would double-scale.
import QtQuick
...
KeyboardStyle {
    ...
    readonly property string fontFamily: LockConfig.menuAreaButtonsFontFamily
    ...
    property color primaryColor: LockConfig.virtualKeyboardPrimaryColor
    property color primaryLightColor: LockConfig.virtualKeyboardKeyActiveBackgroundColor
    property color primaryDarkColor: LockConfig.virtualKeyboardKeyColor
    property color textOnPrimaryColor: LockConfig.virtualKeyboardKeyContentColor
    property color secondaryColor: LockConfig.virtualKeyboardSelectionBackgroundColor
    property color secondaryLightColor: LockConfig.virtualKeyboardPrimaryColor
    property color secondaryDarkColor: LockConfig.virtualKeyboardPrimaryColor
    property color textOnSecondaryColor: LockConfig.virtualKeyboardSelectionContentColor

    property color normalKeyBackgroundColor: primaryDarkColor
    property color highlightedKeyBackgroundColor: primaryLightColor
    property real highlightedKeyBackgroundOpacity: LockConfig.virtualKeyboardKeyActiveOpacity
    property real normalKeyBackgroundOpacity: LockConfig.virtualKeyboardKeyOpacity
    ...
    Component.onDestruction: {
        ...
        // "traceInputGuideConnections" (declared inside a Component further
        // down) was never actually reachable from here — Component bodies
        // are their own id scope. Pre-existing in the original theme too
        // (byte-identical there); dropped instead of ported since it never
        // did anything but throw on every style teardown.
    }
    ...
    property Component component_settingsIcon: Component {
        Item {
            Image {
                ...
                smooth: false
                source: LockConfig.getIcon("settings.svg")
                visible: false
            }
            MultiEffect {
    ...
    keyboardBackground: Rectangle {
        color: "transparent"
        border {
            width: LockConfig.virtualKeyboardBorderSize
            color: LockConfig.virtualKeyboardBorderColor
        }
        radius: 10
        ...
        Rectangle {
            z: parent.z - 1
            anchors.fill: parent
            color: LockConfig.virtualKeyboardBackgroundColor
            opacity: LockConfig.virtualKeyboardBackgroundOpacity
            radius: 10
        }
    }
    ...
    keyPanel: KeyPanel {
        ...
        Rectangle {
            ...
            MouseArea {
                anchors.fill: parent
                // was `!(loginScreen && loginScreen.userNeedsPassword)` —
                // "loginScreen" is an SDDM-only ambient id, unreachable
                // here; the ReferenceError silently forced this (and
                // `enabled`/`hoverEnabled` below it) closed on every key.
                visible: true
                enabled: visible
            ...
            Rectangle {
                anchors.fill: parent
                visible: LockConfig.virtualKeyboardBorderSize > 0
                color: "transparent"
                border {
                    width: LockConfig.virtualKeyboardBorderSize
                    color: LockConfig.virtualKeyboardBorderColor
    ...
    backspaceKeyPanel: KeyPanel {
        ...
        Rectangle {
            ...
            Image {
                ...
                smooth: false
                source: LockConfig.getIcon("backspace.svg")
                visible: false
            }
            MultiEffect {
    ...
    languageKeyPanel: KeyPanel {
        ...
        Rectangle {
            ...
            Image {
                ...
                sourceSize.height: height
                smooth: false
                source: LockConfig.getIcon("language.svg")
                visible: false
            }
            MultiEffect {
    ...
    enterKeyPanel: KeyPanel {
        ...
        Rectangle {
            ...
            Image {
                ...
                sourceSize.height: height
                smooth: false
                source: LockConfig.getIcon("enter-key.svg")
                visible: false
            }
            MultiEffect {
                id: enterIconEffect
    ...
    hideKeyPanel: KeyPanel {
        ...
        Rectangle {
            ...
            Image {
                ...
                sourceSize.height: height
                smooth: false
                source: LockConfig.getIcon("hidekeyboard.svg")
                visible: false
            }
            MultiEffect {
                id: hideIconEffect
            ...
            MouseArea {
                ...
                anchors.fill: parent
                // was `loginScreen.showKeyboard = false` (same unreachable
                // SDDM id) — Qt.inputMethod.hide() is the standard,
                // app-agnostic way to dismiss the active input panel.
    ...
    shiftKeyPanel: KeyPanel {
        ...
        Rectangle {
            ...
            Image {
                ...
                sourceSize.height: height
                smooth: false
                source: LockConfig.getIcon("shift.svg")
                visible: false
            }
            MultiEffect {
                id: shiftKeyColor
            ...
            states: [
                State {
                    ...
                    PropertyChanges {
                        target: shiftKeyIcon
                        source: LockConfig.getIcon("shift-upper.svg")
                    }
                    PropertyChanges {
                ...
                State {
                    ...
                    PropertyChanges {
                        target: shiftKeyIcon
                        source: LockConfig.getIcon("shift-fill.svg")
                    }
                    PropertyChanges {
                        target: shiftKeyColor
                        colorizationColor: primaryColor
    ...
    handwritingKeyPanel: KeyPanel {
        ...
        Rectangle {
            ...
            Image {
                ...
                sourceSize.height: height
                smooth: false
                source: (keyboard && keyboard.handwritingMode) ? LockConfig.getIcon("textmode.svg") : LockConfig.getIcon("handwriting.svg")
                visible: false
            }
            MultiEffect {
                id: hwrIconEffect
    ...
    selectionHandle: Image {
        sourceSize.width: 20
        source: LockConfig.getIcon("selectionhandle.svg")
        ...
        Image {
            ...
            source: {
                ...
                case QtVirtualKeyboard.KeyboardFunction.HideInputPanel:
                    return LockConfig.getIcon("hidekeyboard.svg");
                case QtVirtualKeyboard.KeyboardFunction.ChangeLanguage:
                    return LockConfig.getIcon("language.svg");
                case QtVirtualKeyboard.KeyboardFunction.ToggleHandwritingMode:
                    return keyboard.handwritingMode ? LockConfig.getIcon("textmode.svg") : LockConfig.getIcon("handwriting.svg");
                }
            }
            visible: false
        }
